...
- 下一步建议：保持当前这份风险地图作为现态入口；后续新增审计必须同步更新清单，而不是只追加新文档。
...
- [ ] P1：整理 SDD 文档入口分组。
  ...
  - 范围：`docs/sdd/README.md`
  - 验收：按主链路、基础设施、业务完备度、测试守护、文档口径分组。

- [ ] P2：评估 `DomainPurityTest` 规则分层。
  - 原因：该测试仍有价值，但已经是大型字符串规则清单。
  - 范围：`DomainPurityTest`、`scripts/check-domain-purity.ps1`、`docs/sdd`
  - 验收：明确哪些规则属于粗筛、哪些属于精细架构测试，避免继续无限堆 `contains`。
...
| `DomainPurityTest` 分层治理 | 暂不处理 | 测试缺口 / 文档治理 | P2 | 架构守护可能继续变成大型文本快照测试 | 当前仍能有效挡回归 | 新增大量守护规则前 |
| SDD 文档入口分组 | 未开始 | 文档治理 | P1 | 审计文档继续平铺会增加阅读成本 | 本轮先完成风险排序和清单 | 下一轮继续做文档信息架构治理 |
| 八股文档细粒度短板同步 | 进行中 | 面试口径 | P1 | 面试材料可能落后于 SDD 现态 | 本轮会同步前 5 风险口径 | 每次新增风险排序或主链路结论 |
