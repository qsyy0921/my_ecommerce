...
- 当前为什么还没做：该等待只发生在幂等竞争路径，当前本机规模下没有形成明显功能故障；贸然重构可能扩大改动面。
- 后续触发条件：拼团锁单压测出现明显 RT 抖动，或继续增强锁单幂等策略。
- 下一步建议：先补等待超时语义单元测试，再评估是否把等待策略移到应用层或独立策略对象。
...
- [ ] P0：补拼团锁单等待超时语义单元测试。
  - 原因：这是当前拼团主链路最具体的残留代码风险，先补测试比直接重构更稳。
  - 范围：`TradeLockOrderServiceUnitTest` 及必要 fake port。
  - 验收：覆盖没拿到请求锁、缓存无结果、等待结束后抛 `E0010`。
...
| 拼团锁单等待超时测试 | 未开始 | 测试缺口 / 代码风险 | P0 | 阻塞等待语义没有被单测固化 | 目前只有审计结论，尚未进入代码修改轮次 | 下一轮允许处理主链路测试缺口 |
| 拼团锁单等待策略重构 | 暂不处理 | 代码风险 | P1 | domain service 继续保留 `Thread.sleep` 技术等待 | 需要先补测试，且当前没有功能故障 | 等待超时测试完成，或压测暴露 RT 抖动 |
