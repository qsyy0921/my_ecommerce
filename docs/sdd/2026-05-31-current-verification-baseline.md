...
当前秒杀专业 MQ 演进口径已经更新：Envelope、Outbox 代码闭环和专业 MQ key/tag/partition key 契约已完成，下一步风险收敛到具体 MQ adapter 实现、Outbox 状态台账/告警和真实容量证明。因此需要持续同步 `interview-baguwen.md`、当前风险地图和 SDD 任务清单。
