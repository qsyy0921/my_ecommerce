...
本次先把状态流水的技术细节从 Repository 中移走。后续又持续拆分拼团和秒杀基础设施适配器：拼团侧通知任务、库存流水构建、队伍库存占位、锁单请求锁、结果缓存、锁单落库、结算、退单、读模型、超时扫描和渠道策略都已拆到独立端口，通用 `ITradeRepository` / `TradeRepository` 已删除；秒杀侧库存同步、活动预热、超时未支付释放、Redis 库存预扣、库存流水、结果缓存、订单分片路由、订单命令、活动查询、库存可用性和锁单预扣也已拆到独立端口，通用 `ISeckillRepository` / `SeckillRepository` 已删除。后续收敛重点不再是“大 Repository 删除”，而是补更多领域单元测试、契约测试，以及专业 MQ 演进后的消息契约和容量验证。
