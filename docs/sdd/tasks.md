...
- [x] 秒杀订单 Outbox 支持按状态查询、状态数量台账、重试耗时指标和 Prometheus 告警。
...
- [x] 自定义指标：秒杀订单 Outbox `INIT/SENT/FAILED/DEAD` 数量和自动/人工重试耗时。
- [x] Grafana Dashboard。
...
- [x] Outbox 查询台账和告警完成后刷新当前风险清单，明确第 1 风险已收敛为专业 MQ adapter 演进、秒杀本地技术决策边界和真实容量证明。
...
- [x] 评估 RocketMQ adapter 最小 profile，决策当前单机环境暂不接入具体 MQ 客户端，避免把“能启动”包装成“生产化完成”。
- [x] 抽离秒杀订单号生成端口，`SeckillService` 不再直接使用随机数工具；当前实现保持 12 位数字兼容表结构，并补单 JVM 唯一性测试。
