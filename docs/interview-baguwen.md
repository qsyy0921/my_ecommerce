...
- 支付商城是交易入口：负责商品下单、支付单、支付宝/模拟支付、支付回调、退款入口、订单列表、支付流水、退款流水、三方账单导入和对账差错处理。
- 拼团营销服务是营销规则中心：负责活动配置、优惠试算、人群标签、拼团队伍、拼团锁单、秒杀锁单、成团结算、退单恢复、通知任务和补偿任务。
...
- 库存流水已有 `RESERVE/ROLLBACK` 审计，但支付成功、退款成功、库存是否重新开放需要更完整的状态流。
- 如果面试官问“秒杀支付后退款是否恢复库存”，要诚实说目前重点做了抢购资格和超时释放，售后库存策略还可以继续增强。
