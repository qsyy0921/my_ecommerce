...
- 对外展示名使用 `qsyy-ecommerce-platform`，维护者为 qsyy。
- Java package `cn.bugstack`、`xfg-wrench-*` 外部依赖坐标、`group_buy_market` 数据库/MQ/Redis 运行标识暂保留，面试时可解释为“兼容历史代码和本地环境，不为展示名迁移破坏运行链路”。
...
- 项目命名迁移只完成了可见标识：README、POM 开发者、Dockerfile maintainer、前端展示和样例用户已经统一到 qsyy；package、外部依赖坐标、数据库/MQ/Redis 运行标识暂不改，避免引入低收益高风险的大范围迁移。
