...
const AppUtils = {
    ...
    getPayChannel: function() {
        return localStorage.getItem("payChannel") || AppConfig.defaultPayChannel;
    },

    setPayChannel: function(channel) {
        localStorage.setItem("payChannel", channel);
        document.querySelectorAll('[data-pay-channel-option]').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.payChannelOption === channel);
        });
    }
};
