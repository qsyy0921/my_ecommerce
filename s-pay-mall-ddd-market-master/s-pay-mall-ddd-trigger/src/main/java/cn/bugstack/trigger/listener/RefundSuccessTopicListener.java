...
/**
 * Refund success message listener.
 */
@Slf4j
@Component
public class RefundSuccessTopicListener {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "${spring.rabbitmq.config.consumer.topic_team_refund.queue}", durable = "true",
                            arguments = {
    ...
    public void listener(String message, Message amqpMessage, Channel channel) throws Exception {
        ...
        try {
            ...
            log.info("receive refund callback messageId:{} body:{}", messageRecord.getMessageId(), message);
            ...
            log.error("consume refund callback failed messageId:{} body:{}", messageId, message, e);
